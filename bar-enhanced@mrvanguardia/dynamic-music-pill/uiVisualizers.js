...
const CavaVisualizer = GObject.registerClass(
    class CavaVisualizer extends St.DrawingArea {
        ...
        _updateSubscription() {
            let shouldSubscribe = this.mapped && this._isPlaying;
            if (shouldSubscribe) {
                if (!this._subscribed) {
                    this._engine.subscribe(this._engineCallback);
                    this._subscribed = true;
                }
                this._engine.setPlaying(this._engineCallback, this._isPlaying);
            } else {
                if (this._subscribed) {
...
const SimulatedVisualizer = GObject.registerClass(
    class SimulatedVisualizer extends St.BoxLayout {
        ...
        _updateTimer() {
            ...
            if (this._isPlaying && this.mapped && this._mode !== 0) {
